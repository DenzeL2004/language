...
На вход программы подается текст программы на языке "dog". Перед проведением синтаксического анализа происходит синтаксический анализ. Цель анализа - разделить строки входного файла на токены. Всего есть три тиипа токеов:

1. **Слово**. Слово обязано начинаться с заглавной или прописной латинской буквы, а также символа нижнее подчеркивание "_". После этого в слове могут идти только латинские буквы верхнего и нижнего регистра, цифры и символ нижнее подчеркивание. Если встертился пробельный символ или символ, не принадлежащий к ранее описанным, то этот сивол не идет в токен.

2. **Число**. Число обязано начинаться с цифры. После этого могут идти цифры или точка "." (причем ровно один символ точки). Если встертился пробельный символ или символ, не принадлежащий к ранее описанным, то этот сивол не идет в токен.

3. **Специальный символ**. К специальным сиволам относяться сиволы, которые не огут быть началом **Числа** или **Слово**.
...
- `read()`  - возвращает считанное число.
- `main()`  - возвращает результат исполнения прграммы.
